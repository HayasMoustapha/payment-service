...
/**
 * Middleware de vérification de webhook Stripe
 */
function requireStripeWebhook() {
  return (req, res, next) => {
    const signature = req.headers['stripe-signature'];
    const webhookSecret = process.env.STRIPE_WEBHOOK_SECRET;

    if (!signature || !webhookSecret) {
      logger.security('Stripe webhook signature missing', {
      ...
      return res.status(401).json(
        errorResponse('Signature de webhook Stripe manquante', null, 'MISSING_WEBHOOK_SIGNATURE')
      );
    }

    // La vérification sera faite dans le contrôleur avec le payload
    req.stripeSignature = signature;
    
    logger.auth('Stripe webhook signature received', {
      ip: req.ip
    });
...
/**
 * Middleware de vérification de webhook PayPal
 */
function requirePayPalWebhook() {
  return (req, res, next) => {
    ...
    if (!paypalAuthAlgo || !paypalCertId || !paypalTransmissionId || 
        !paypalTransmissionSig || !paypalTransmissionTime) {
      logger.security('PayPal webhook headers missing', {
        hasAuthAlgo: !!paypalAuthAlgo,
        hasCertId: !!paypalCertId,
        hasTransmissionId: !!paypalTransmissionId,
        hasTransmissionSig: !!paypalTransmissionSig,
        hasTransmissionTime: !!paypalTransmissionTime,
        ip: req.ip
      });

      return res.status(401).json(
        errorResponse('Headers de webhook PayPal manquants', null, 'MISSING_WEBHOOK_HEADERS')
      );
    }

    // Stocker les headers pour la vérification dans le contrôleur
    req.paypalWebhookHeaders = {
      'paypal-auth-algo': paypalAuthAlgo,
      'paypal-cert-id': paypalCertId,
    ...
    logger.auth('PayPal webhook headers received', {
...
/**
 * Middleware de validation de propriétaire de ressource
 * @param {string} resourceType - Type de ressource
 * @param {string} resourceIdParam - Nom du paramètre ID
 */
function requireResourceOwner(resourceType, resourceIdParam = 'id') {
  return (req, res, next) => {
    if (!req.user?.id) {
      return res.status(401).json(
        errorResponse('Authentification requise', null, 'NOT_AUTHENTICATED')
    ...
    if (!resourceId) {
      return res.status(400).json(
        errorResponse('ID de ressource requis', null, 'MISSING_RESOURCE_ID')
      );
    }

    // Pour l'instant, on passe la validation
    // Dans une implémentation complète, cela vérifierait la base de données
    req.resourceOwnerId = req.user.id;
    
    logger.auth('Resource ownership validated', {
      userId: req.user.id,
      resourceType,
      resourceId,
      ip: req.ip
